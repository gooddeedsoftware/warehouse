        ...
        <table class="table table-sm" style='font-size: 11px !important;'>
			<thead>
				<tr style="background-color: lightgrey !important; font-style:italic !important">
					...
					<th scope="col" width="18%">{{ __('report.lager_location')  }}</th>
					<th scope="col" width="8%">{{ __('report.available')  }}</th>
					<th scope="col" width="13%">{{ __('report.delivery_date')  }}</th>
					<th scope="col" width="13%">{{ __('report.delivered')  }}</th>
					<th scope="col" width="8%">{{ __('report.leveres')  }}</th>
					<th scope="col" width="8%">{{ __('report.Plukket')  }}</th>
					<th scope="col" width="7%">{{ __('report.rest')  }}</th>
				</tr>
			</thead>
			<tbody>
				@if(@$order_materials)
						...
						<tr>
							...
							<td>
								@if($product->is_package != 1)
									{{ $product->pick_warehouse }} / {{ $product->pick_location_id }}
								@endif
							</td>
							<td>
								@if($product->is_package != 1)
									{!! number_format(@$product->available_qty, 2, ",", "") !!}
								@endif
							</td>
							<td>
							...
							<td>
								@if($product->is_package != 1)
									{!! number_format(@$product->delivered_qty, 2, ",", "") !!}
								@endif
							</td>
							<td>
							 	{!! number_format(@$product->order_quantity, 2, ",", "") !!}
							</td>
							<td> <u> &emsp;&emsp;&emsp;</u></td>
							<td> <u> &emsp;&emsp;&emsp;</u></td>
						</tr>

						@if ($product->is_package == 1 && @$product->package_contents)
							@foreach($product->package_contents as $package_product)
								<tr>
									...
									<td>
										{{ $package_product->pick_warehouse }} / {{ $package_product->pick_location_id }}
									</td>
									<td>
										{!! number_format(@$package_product->available_qty, 2, ",", "") !!}
									</td>
									<td>
										{{  @$package_product->delivery_date ? date('d.m.Y', strtotime($package_product->delivery_date)) : '' }}
									</td>
									<td>
										{!! number_format(@$package_product->delivered_qty, 2, ",", "") !!}
									</td>
									<td>
