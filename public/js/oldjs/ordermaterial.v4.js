...
$(".add_product_material").click(function() {
    ...
    htmlString += "<td class='product_td'><select style='width:100% !important;' class='product_number_select2 select2 form-control product product_number' onchange='productOnchnage(this, \"1\", \"" + usertype + "\",\"" + user_warehouse_resposible + "\", \"" + user_warehouse_resposible_id + "\");'>" + product_options + "'</select><input class='labelProduct form-control hide_div' /></td>";
    ...
    htmlString += "<td class='warehouse_td'><select class='select2 form-control warehouse' onchange='getProductDetailsForMaterials(this,  \"1\", \"" + usertype + "\",\"" + user_warehouse_resposible + "\", );'>" + warehouse_options + "</select><label class='labelWarehouse hide_div'>test</label></td>";
    htmlString += "<td class='location_td'><select class='select2 form-control location ' onchange='showPickedQuantity(this, 1);'>" + location_options + "</select><label class='labelLocation hide_div'>test</label>";
    ...
    htmlString += "<td class='info_td'><a class='stock_info_btn hide_div' type='button' onclick='showStockInfo(this);' unique_id=" + unique_id + "><i class='fa fa-info-circle'></i></a></td>";
...
function saveOrderMaterial(obj, material_reference_id, type) {
    ...
    $.ajax({
        ...
        data: {
            ...
            'product_description': $(obj).parent('td').siblings('.product_td').find('.product_number').find(":selected").text(),
...
function updateOrderMaterialData(obj, type) {
    ...
    $.ajax({
        ...
        data: {
            ...
            'price': $(obj).closest('tr').find('.price').val(),
            'cost_price': $(obj).closest('tr').find('.cost_price').val(),
            'product_description': $(obj).closest('tr').find('.labelProduct').val(),
            'dg': $(obj).closest('tr').find('.dg').val(),
        },
        success: function(response) {
            decoded_response = $.parseJSON(response);
...
function generateProductPackageRow(product_package) {
    ...
    if (product_package) {
        var unique_id = createUUID();
        var htmlString = "<tr class='order_material_tr' data-val='" + unique_id + "'>";
        ...
        if (product_package['package_products'] != undefined && product_package['package_products'].length > 0) {
            for (var i = 0; i < product_package['package_products'].length; i++) {
                ...
                var contents_html = "<tr class='order_material_tr child_products' data-val='" + unique_id + "'><input type='hidden' id='is_content' value='1' /><td></td>";
                var product_options = "";
...
function hideOrShowPack() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (decoded_response.data.shipping_data >= 1 && decoded_response.data.picked_product >= 1) {
                $('#packlist').show();
            }
