 ...
 $(".add_product_material").click(function() {
     ...
     htmlString += "<td class='product_td'><select style='width:100% !important;' onchange='loadProductDetails(this)' class='product_number_select2 select2 form-control product product_number'>" + product_options + "'</select><input class='labelProduct form-control hide_div'/></td>";
 ...
 function saveOrderMaterial(obj) {
     ...
     if (product_id) {
         var product_description = $(obj).parent('td').siblings('.product_td').find('.product_number').find(":selected").text();
         var material_id = $(obj).closest('tr').attr('material_id');
         if (material_id) {
